local SPARKLE_TEXTURE = resolvefilepath("images/fx/hh.tex")

local ADD_SHADER = "shaders/vfx_particle_add.ksh"

local COLOUR_ENVELOPE_NAME = "cane_hh_colourenvelope"
local SCALE_ENVELOPE_NAME = "cane_hh_scaleenvelope"
...
local function InitEnvelope()
    ...
    while t + step + .01 < 1 do
        table.insert(envs, { t, IntColour(153, 204, 153, 255) })
        t = t + step
        table.insert(envs, { t, IntColour(153, 204, 153, 0) })
        t = t + .01
    end
    table.insert(envs, { 1, IntColour(153, 204, 153, 0) })
...
--------------------------------------------------------------------------
local MAX_LIFETIME = 1.6

local function emit_sparkle_fn(effect, sphere_emitter)
    local vx, vy, vz = .012 * UnitRand(), .016 * UnitRand(), .012 * UnitRand()
    ...
    effect:AddRotatingParticleUV(
        0,
        lifetime,                             -- lifetime
        px, py + math.random(1.56, 1.66), pz, -- position
        vx, vy, vz,                           -- velocity
...
local function fn()
    ...
    local sparkle_desired_pps_low = 16
    local sparkle_desired_pps_high = 66
    local low_per_tick = sparkle_desired_pps_low * tick_time
    local high_per_tick = sparkle_desired_pps_high * tick_time
    local num_to_emit = 0

    local sphere_emitter = CreateSphereEmitter(.36)
...
return Prefab("cane_hh_fx", fn, assets)
